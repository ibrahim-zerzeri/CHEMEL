...
// Step 2: Fetch Subjects with Optional Search Term
$searchTerm = isset($_GET['search']) ? $_GET['search'] : ''; // Get search term from URL

try {
    if (!empty($searchTerm)) {
        // Search for subjects using a prepared statement
        $stmt = $conn->prepare("SELECT * FROM subjects WHERE name LIKE :search");
        $stmt->execute(['search' => '%' . $searchTerm . '%']);
    } else {
        // Fetch all subjects if no search term is provided
...
<div class="container my-4">
    <form method="GET" action="" class="form-inline justify-content-center">
        <input type="text" name="search" class="form-control mr-2" placeholder="Search subjects" value="<?= htmlspecialchars($searchTerm) ?>">
